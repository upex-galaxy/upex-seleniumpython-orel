...
class WebTablesPage(Page):
    def __init__(self, driver: WebDriver):
        ...
        self.first_name_header = lambda: self.locators.get_by_text('First Name')[0]
        self.last_name_header = lambda: self.locators.get_by_text('Last Name')[0]
        self.age_header = lambda: self.locators.get_by_text('Age')[0]
        self.email_header = lambda: self.locators.get_by_text('Email')[0]
        self.salary_header = lambda: self.locators.get_by_text('Salary')[0]
        self.department_header = lambda: self.locators.get_by_text('Department')[0]
    ...
    def get_edit_button(self, value: str):
        edit_button = self.locators.get_parent_by_text(value)
        return edit_button.find_element(By.CSS_SELECTOR, 'span[id*="edit-record"]')

    # Ubica el boton delete de un registro especifico en la tabla
    def get_delete_button(self, value: str):
        delete_button = self.locators.get_parent_by_text(value)
        return delete_button.find_element(By.CSS_SELECTOR, 'span[id*="delete-record"]')
    ...
    def open_web_tables_page(self):
        ...
        self.locators.wait_for_element(page_button)
    ...
    def add_new_record(self, first_name: str, last_name: str, email: str, age: int, salary: int, department: str):
    ...
    # Acciones (Agregar multiples registros de prueba en la tabla de registros)

    def add_multiple_records(self, record_count=8):
    ...
    # Acciones (Filtrar la cantidad de registros que se mostraran en la tabla (5,10,20,25,50,100))

    def set_visible_row_count(self, value):
        """Filtrar la cantidad de registros que se mostraran en la tabla (5,10,20,25,50,100"""
        dropdown = Select(self.web.find_element(By.CSS_SELECTOR, ".select-wrap select"))
